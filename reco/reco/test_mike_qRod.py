...
if __name__ == '__main__':
	...
	#set test_x based on model: 8:24 for allchan, 24:32 for avg, 24:26 for CoM
        ...
        if do_ratio_plot_ptnuc:
                ...
                psi_rec_A_2 = np.arctan2(Q_predicted_A_2[:,1],Q_predicted_A_2[:,0])
                psi_gen_res_model2 = process.GetPsiResidual_np(psi_gen_A, psi_rec_A_2)
                psi_truth_res_model2 = process.GetPsiResidual_np(psi_truth_A, psi_rec_A_2)
        ...
        if do_truth_pos_plot:
                vis_root.PlotTruthPos(test_A[:,0], test_A[:,1], output_path)
        if do_subtracted_channel_plot:
                vis_root.PlotSubtractedChannels(test_A[:,8:24], output_path)
        if do_ratio_plot_ptnuc:
                vis_root.PlotRatio_ptnuc_hist(pt_nuc_A, psi_gen_rec_A, psi_gen_res_model2, upperRange_gen, model_type_1 , model_type_2, output_path, is_gen = True, save_residuals = False)
#                vis_root.PlotRatio_ptnuc(pt_nuc_A, psi_gen_rec_A, psi_truth_res_model2, upperRange_gen, model_type_1 , model_type_2, output_path, save_residuals = False)
        if do_position_resolution:
                ...
                vis_root.PlotPositionRes(test_A[:,0], test_A[:,1], psi_gen_rec_A, model_type_1, output_path, is_gen = True, save_residuals = True)
